...
class KnowledgeGraphDaemon:
    ...
    def _process_unmapped_sources(self):
        ...
        for topic_info in completed_topics:
            ...
            if sources:
                logger.info(
                    f"Found {len(sources)} unmapped sources for topic '{topic_name}' in database: {'local' if not database_uri else database_uri}"
                )
    ...
    def _get_unmapped_sources_for_topic(
        self, database_uri: str, topic_name: str
    ) -> List[Dict]:
        """
        ...
        with session_factory() as db:
            ...
            result = db.execute(query, {"topic_name": topic_name}).fetchall()
    ...
    def _build_graph_for_topic(
        self, topic_name: str, sources: List[Dict], database_uri: str
    ):
        """
        ...
        if db_manager.is_local_mode(database_uri):
            session_factory = SessionLocal
            logger.info(
                f"Building knowledge graph for local topic: {topic_name} with {len(sources)} sources"
            )
        else:
            session_factory = db_manager.get_session_factory(database_uri)
            logger.info(
                f"Building knowledge graph for external topic: {topic_name} with {len(sources)} sources"
            )
        ...
        try:
            ...
            logger.error(
                f"Knowledge graph building failed for topic {topic_name}: {str(e)}",
                exc_info=True,
            )
            raise
        ...
        try:
            result = graph_builder.enhance_knowledge_graph(
                topic_name, extracted_sources
            )
            logger.info(
                f"Successfully enhanced knowledge graph for topic: {topic_name}"
            )
    ...
    def get_daemon_status(self) -> Dict:
        ...
        for topic_info in completed_topics:
            ...
            database_uri = topic_info["database_uri"]
            topic_key = (
                f"{topic_name} ({'local' if not database_uri else database_uri})"
            )

            try:
                unmapped_count = len(
                    self._get_unmapped_sources_for_topic(database_uri, topic_name)
                )
                topic_stats[topic_key] = unmapped_count
                total_unmapped += unmapped_count
            except Exception as e:
                logger.error(
                    f"Error getting stats for topic {topic_name} in database {database_uri}: {e}"
                )
        ...
        try:
            with SessionLocal() as db:
                ...
                entity_mappings = (
                    ...
                    .filter(SourceGraphMapping.graph_element_type == "entity")
                    .count()
                )
                relationship_mappings = (
                    db.query(SourceGraphMapping)
                    .filter(SourceGraphMapping.graph_element_type == "relationship")
                    .count()
                )
